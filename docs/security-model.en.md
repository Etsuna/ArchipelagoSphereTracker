...
|---|---|---|
| Guild member | read operations, personal recap and alias, upload and analysis of the room's shared spoiler; all Archipelago tools unless explicitly denied | Discord member who can still access the channel or thread |
| Room manager | room configuration/deletion, thread portal, patch listing | thread owner, `Manage Threads`, or guild manager |
| Guild manager | room creation and management, global portal | guild owner, administrator, `Manage Server`, delegated AST manager, or instance owner |
...
In Archipelago mode, YAML, APWorld, generation, and template operations are available to every guild member by default. A guild owner, administrator, member with `Manage Server`, or the instance owner can explicitly deny or restore a member through `/ast` → `AST administration` → `Archipelago access restrictions`. The restriction is persisted per guild, audited, and enforced for `/ast`, the legacy direct slash commands, and corresponding Web operations. Delegated AST managers cannot edit the deny list. The configured instance owner cannot be denied.
...
- APWorld files contain executable code. They are unavailable in Normal mode. In single-guild Archipelago mode, they follow the default-open, explicit-deny policy above. File validation reduces malformed archives but does not establish that embedded code is trustworthy.
